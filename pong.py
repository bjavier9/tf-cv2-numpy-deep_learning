...
icon_path = os.path.join( u'1.ico')
...
pygame.display.set_caption('IA vs cpu')
...
def drawInfos(infos, action):
    ...
    label = font.render("Tiempo de uso: " + str(infos[0]), 1, WHITE)
    ...
    if (action[2] == 1):
        actionText = "Abajo"
    label = font.render("precionando: " + actionText, 1, WHITE)
...
class pongGame:
    ...
    def getNextFrame(self, action, infos):
        ...
        drawball(self.bolaXpos, self.bolaYpos)
        image_data = pygame.surfarray.array3d(pygame.display.get_surface())
        drawScore(self.puntos)
        drawInfos(infos, action)
        pygame.display.flip()
        self.puntos = self.puntos + score
